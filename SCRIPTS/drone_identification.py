...
from SCRIPTS.DroneManager import DroneManager
...
# y=np.array([chirp(x[i],1,0,10) for i in range(len(x))])
#
# plt.figure(1)
# plt.plot(x,y)
# plt.show()
...
with SyncCrazyflie(sc_v.uri, sc_s.cf) as scf:
    ...
    drone.start(0.0, 0.3)
    for i in range(20):
        drone.scf.cf.commander.send_position_setpoint(-0.5,-0.5,sc_v.DEFAULT_HEIGHT,90)
        time.sleep(0.1)
    while times < t_tot:
        # drone.send_command(0.5, 0.0, 0.0, 0.05)
        omega_comm = 90 * chirp(times, 5, 0, t_tot)
        vx_comm = 0.0#chirp(times, 5, 0, t_tot)
        drone.send_command(vx_comm, 0.0, omega_comm, dt)

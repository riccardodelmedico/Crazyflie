...
class Fading_Filter:
    ...
    def stop(self):
        ...
        # print(self.real_pos.shape[0],self.list_pos.shape[0],self.time_line.shape[0])
        # self.time_line= self.time_line[0:self.list_pos.shape[0]-1]
    ...
    def plot_Filter(self):
        ...
        if not self.target.wand:
            ...
            sp.suptitle('Estimation vs Real Velocity')
            for i in range(3):
                ax[i].plot(self.time_line, self.list_vel[:, i], '--b',
                              label=f'estimate {legend[1, i]}')
                ax[i].legend()
            plt.show()
            if self.order == 3:
                sp, ax = plt.subplots(3,1)
